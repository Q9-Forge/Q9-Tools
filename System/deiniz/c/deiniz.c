...
static void usage(void){fputs("Syntax: deiniz [<opts>] {<devname>}\015Function: detach devices\015     -z[=]file  read device names from file/stdin\015",stdout);}
static int one(const char *name){void *d=attach(name,0);if(!d){fprintf(stderr,"can't access '%s'\015",name);return 1;}if(detach(d)!=0){fprintf(stderr,"can't detach '%s'\015",name);return 1;}return 0;}
